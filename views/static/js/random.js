...
function gotPoses(poses) {
  ...
  if (poses.length > 0) {
    ...
    if (state == 'collecting') {
      console.log(state)
      let inputs = [];
      for (let i of [5,6,11,12,13,14,15,16]) {
        let x = pose.keypoints[i].position.x-pose.keypoints[11].position.x;
        let y = pose.keypoints[i].position.y-pose.keypoints[11].position.y;
        inputs.push(x);
        inputs.push(y);
      }
